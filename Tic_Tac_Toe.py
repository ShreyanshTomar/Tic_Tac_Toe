...
def userdata():
    Position = input("Enter the position you want to mark your symbol on:")
...
def start_tic_tac_toe():
    global count
    global board
    print color.RED + "Welcome to Tic_Tac_Toe!" + color.END
    Board()
    for turn in xrange(1,10):
        if turn%2 == 1:
            ...
            if(board[0]==board[1]==board[2] or board[0]==board[3]==board[6] or board[2]==board[5]==board[8] or board[6]==board[7]==board[8] or board[3]==board[4]==board[5] or board[0]==board[4]==board[8] or board[2]==board[4]==board[6]):
                print 'Player 1 wins  _へ__(‾◡◝ )>'
                print '＼（＾○＾）人（＾○＾）／'
                print 'Player 2 loses ⊙﹏⊙'
                break
            else:
                count += 1
                pass
